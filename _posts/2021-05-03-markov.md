...
This is equal (the one that was calculated manually was rounded to 4-5 decimal places, so it is off by ~0.001), to the probability found by computing the probability of every possible state sequence seperately and getting the sum (as seen in the figure showing the probability distribution above). It is more efficient however.
