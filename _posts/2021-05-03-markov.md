...
##### Forward Probability
The Forward Probability is the probability of being in state i at time t, and being in state j at time t+1, for a given observation sequence and model. 
...
where:
- \\(\\alpha_{i}(t)\\) is the probability of being in state i and seeing symbol \\(x_{i}\\) at time t. This is the value that is returned.
